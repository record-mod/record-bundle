...
const RECORD_METRO_CACHE_PATH = "caches/metro_modules.json";
...
export async function initMetroCache() {
    if (!(await fileExists(RECORD_METRO_CACHE_PATH)))
        return void buildInitCache();
    const rawCache = await readFile(RECORD_METRO_CACHE_PATH);
    ...
    try {
        ...
        if (
            _metroCache._buildNumber !==
            NativeClientInfoModule.getConstants().Build
        ) {
            _metroCache = null!;
...
const saveCache = debounce(() => {
    writeFile(RECORD_METRO_CACHE_PATH, JSON.stringify(_metroCache));
}, 1000);
...
export function getPolyfillModuleCacher(name: string) {
    ...
    return {
        getModules() {
            return require("@metro/internals/modules").getCachedPolyfillModules(
                name
            );
        },
