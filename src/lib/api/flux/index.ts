...
const blockedSym = Symbol.for("record.flux.blocked");
const modifiedSym = Symbol.for("record.flux.modified");
...
export function injectFluxInterceptor() {
    ...
    return () =>
        (dispatcher._interceptors &&= dispatcher._interceptors.filter(
            (v) => v !== cb
        ));
}
