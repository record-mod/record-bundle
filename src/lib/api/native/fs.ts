 ...
 */
export async function removeFile(path: string, { prefix = "record/" } = {}) {
    if (typeof NativeFileModule.removeFile !== "function")
        throw new Error("'fs.removeFile' is not supported");
    return void (await NativeFileModule.removeFile(
        "documents",
        `${prefix}${path}`
    ));
}
 ...
 * @param path Path to the file
 */
export async function removeCacheFile(path: string, prefix = "record/") {
    if (typeof NativeFileModule.removeFile !== "function")
        throw new Error("'fs.removeFile' is not supported");
    return void (await NativeFileModule.removeFile(
        "cache",
 ...
 * @param path Path to the file
 */
export async function fileExists(path: string, { prefix = "record/" } = {}) {
    return await NativeFileModule.fileExists(
...
export async function writeFile(
    ...
    return void (await NativeFileModule.writeFile(
        "documents",
        `${prefix}${path}`,
        data,
        "utf8"
...
export async function readFile(
    ...
    try {
        return await NativeFileModule.readFile(
            `${
                NativeFileModule.getConstants().DocumentsDirPath
            }/${prefix}${path}`,
            "utf8"
        );
    } catch (err) {
...
export async function downloadFile(
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to download file from ${url}: ${response.status}`
        );
    }
    ...
    await NativeFileModule.writeFile(
        "documents",
        `${prefix}${path}`,
        data,
        "base64"
